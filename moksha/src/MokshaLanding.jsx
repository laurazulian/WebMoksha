...
export default function MokshaLanding() {
  ...
  return (
    <div className="font-sans text-[#333] bg-[#f8f4f0] scroll-smooth">
      ...
      {/* SECCIÓN DE MENÚ */}
      <section id="productos" className="py-16 px-4 md:px-12">
        <div className="max-w-3xl mx-auto grid gap-16">
          {productos.map((grupo, idx) => (
            <div key={idx}>
              <h2 className="text-xl font-serif text-[#747c3c] border-b border-gray-300 pb-1 mb-4 uppercase tracking-wider">
                {grupo.categoria}
              </h2>
              <ul className="space-y-4">
                {grupo.items.map((item, i) => (
                  <li
                    key={i}
                    className="flex justify-between items-start transition-transform duration-300 hover:scale-105"
                  >
                    <div>
